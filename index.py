...
def check_eligibility(data):
	...
	#Eligibility perfect
		...
		sql = "SELECT student_id FROM student WHERE gpa >= 3 AND records = 'n' AND student_id = %s"
		
		mycursor.execute(sql,exe)
		
		perfect = len(mycursor.fetchall())
		
	#Eligibility semi-perfect	
		sql = "SELECT student_id FROM student WHERE (gpa >=3 AND records = 'y') OR (gpa <= 3 AND records = 'n') AND student_id = %s"
		
		mycursor.execute(sql,exe)
		
		semiperfect = len(mycursor.fetchall())
		
	#Eligibility notperfect	
		sql = "SELECT student_id FROM student WHERE gpa <= 3 AND records = 'y' AND student_id = %s"
...
def check_intregistration(data):
	...
	if counting >= 1:
		...
		if have >= 1:
			response = "You have already registered! student " + re.sub(r'[^\w\s]','',str(myresult)) + "☺️"
			reply = {
				"fulfillmentText": response,
				"fulfillmentMessages": [
			{
			...
			response = "You have yet to register! you should head over to (Register Now) to register for overseas internship 😁"
			reply = {
				"fulfillmentText": response,
				"fulfillmentMessages": [
			{
...
#______________________________________________________________________________________________________________________________________________________________	
	...
	#For students who are not in iprep
		...
		if haveiprep >= 1:
			response = "You ARE in iprep student " + re.sub(r'[^\w\s]','',str(myresult)) + " 👻"
			reply = {
				"fulfillmentText": response,
				"fulfillmentMessages": [
			{
			...
			response = "You are NOT in iprep student " + re.sub(r'[^\w\s]','',str(myresult)) + " 👻"
			reply = {
				"fulfillmentText": response,
				"fulfillmentMessages": [
			{
